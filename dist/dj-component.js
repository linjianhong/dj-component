  ...
  var theControlers = {
    ...
    "dropdown": ["$scope", "$timeout", "$http", "$q", "DjWaiteReady", function ($scope, $timeout, $http, $q, DjWaiteReady) {
      ...
      $scope.click = function (item) {
        $scope.selected = item;
        _this13.onChange({ value: item.value || item });
      };
...
(function () {
  ...
  function ctrl($scope, $http, IMG, DjPop, DjDialog) {
    ...
    this.deleteImg = function (n) {
      ...
      return DjDialog.confirm("您确认要删除当前图片?").then(function (a) {
